...
# Streamlit App UI
# -------------------------
st.set_page_config(page_title="Enterprise Management System", page_icon="🏢", layout="wide")
st.title("🏢 Enterprise Management System")
st.markdown("**HR | Finance | Procurement | CRM** – Streamlit Professional Dashboard Demo")

# Sidebar
module = st.sidebar.radio(
    "Navigation",
    ["Overview","HR","Finance","Procurement / Pending","CRM","Data Import/Export","PDF Snapshot"]
)
st.sidebar.markdown("---")
st.sidebar.caption("Enterprise WebApp | SQLite backend | Streamlit UI")
...
if module=="Overview":
    ...
    col1.metric("👨‍💼 Employees", emp_count)
    col2.metric("👥 Customers", cust_count)
    col3.metric("🛒 Pending POs", pending_pos)
...
# ---------- HR ----------
    ...
    with hr_tab[0]:  # Employees
        st.subheader("➕ Add New Employee")
        with st.expander("Employee Form"):
            with st.form("add_emp_form"):
                col1, col2 = st.columns(2)
                emp_code = col1.text_input("Employee Code")
        ...
        emps = query_df("SELECT * FROM employees")
        st.markdown("### Existing Employees")
        st.dataframe(emps.style.highlight_max(axis=0, color="lightgreen"))

    with hr_tab[1]:  # Attendance
        st.subheader("📌 Mark Attendance")
        emps = query_df("SELECT id, name, emp_code FROM employees")
        if emps.empty:
            st.info("No employees - add employees first.")
            ...
            with st.form("apply_leave"):
                ...
                if apply_btn:
                    ...
                    if not emp_row.empty:
                        emp_id = int(emp_row.iloc[0]['id'])
                        insert_and_commit(
                            "INSERT INTO leaves(emp_id,leave_type,start_date,end_date,reason,applied_on) VALUES (?,?,?,?,?,?)",
                            (emp_id, ltype, start.isoformat(), end.isoformat(), reason, datetime.date.today().isoformat())
                        )
                        st.success("✅ Leave applied (Pending approval)")
        leaves = query_df("SELECT l.id, e.emp_code, e.name, l.leave_type, l.start_date, l.end_date, l.reason, l.status FROM leaves l JOIN employees e ON l.emp_id = e.id WHERE l.status='Pending'")
    ...
    with hr_tab[3]:  # Reports
        st.subheader("📊 HR Reports")
        emps = query_df("SELECT * FROM employees")
        st.markdown("**Headcount by Department**")
        if not emps.empty:
            st.dataframe(emps.groupby("department")["id"].count().rename("count").reset_index())
        st.markdown("**Absences last 30 days**")
        abs_df = query_df("SELECT a.date, e.emp_code, e.name, a.status FROM attendance a JOIN employees e ON a.emp_id=e.id WHERE a.status='Absent' AND date(a.date) >= date('now','-30 days')")
        st.dataframe(abs_df.style.applymap(lambda v: 'background-color: #ffcccc' if v=='Absent' else '', subset=['status']))
...
# ---------- Procurement / CRM / Data / PDF ---------- 
# (Abbreviated here for brevity, same refactor pattern: expander forms + tabs + styled tables)

st.markdown("---")
st.caption("Streamlit Enterprise Management System | Professional UI Demo | SQLite backend")
